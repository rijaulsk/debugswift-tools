 ...
 *
 * FOURTH basePath trap, and the reason the mask URL goes through publicAsset()
 * instead of being written literally. basePath is applied by next/link, the
 * router, and the image optimizer — it is NOT applied to a URL inside a CSS
 * value. `url(/brand/…)` therefore resolves against the domain root, where this
 * deployment serves nothing, and the footer mark silently disappears.
 *
 * It is written as a computed call rather than a hand-typed "/tools/brand/…"
 * on purpose: this file is copied between the three repos, and a hardcoded
 * prefix is exactly the kind of thing a re-copy overwrites without anyone
 * noticing. publicAsset() reads the right prefix for whichever repo it is in. */
